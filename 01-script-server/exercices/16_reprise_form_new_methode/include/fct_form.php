<?php
/*
La fonction "init_tagname" est une fonction qui permet d'initialiser 
et de récupérer des données provenant d'un formulaire
en utilisant une superglobale ($_POST ou $_GET) 
et un array de nom de champs.

- Le premier paramètre, $array_name, 
est un array associatif contenant le nom des champs 
et le type de données attendues pour chaque champ.

- Le deuxième paramètre facultatif, $superglobale, 
définit la superglobale utilisée pour récupérer les données 
(par défaut "POST").
...
La fonction utilise les filtres de PHP pour nettoyer et 
vérifier les données avant de les stocker dans des variables globales 
avec le nom des champs
préfixées avec $prefixe.

La fonction effectue les étapes suivantes :

-   Vérification de la superglobale : La fonction vérifie 
si la superglobale mentionnée en paramètre ($superglobale) 
est valide (soit "POST" ou "GET").
    Si ce n'est pas le cas, elle affiche une erreur et 
    quitte l'exécution du script.
...
-   Boucle sur les champs :
    ...
    Les variables sont initialisées avec le nom du champ 
    préfixé avec le préfixe défini. Si le type de champ est un tableau,
    la fonction utilise :
    *   le filtre FILTER_REQUIRE_ARRAY pour récupérer 
    les données sous forme de tableau.
    *   Sinon, elle utilise un filtre spécifique en fonction 
    du type de champ attendu (FILTER_SANITIZE_SPECIAL_CHARS pour "string",
        FILTER_SANITIZE_NUMBER_INT pour "int" et 
        FILTER_SANITIZE_NUMBER_FLOAT pour "float").
*/
function init_tagname($array_name, $superglobale = "POST", $prefixe = ""){
    // la valeur de $superglobale est mise en majuscules 
    // au cas où elle aurait été déclarée en minuscules
    $superglobale = strtoupper($superglobale);
    // un tableau comprenant les superglobales autorisées 
    // dans le cas de cette fonction
    $sg = ["POST", "GET"];
    // on vérifie si la superglobale spécifiée fait bien 
    // partie des superglobales autorisée sinon on arrête 
    // le script et affiche une erreur
    if(!in_array($superglobale, $sg)){
        exit("Erreur d'initialisation ! La superglobale que vous 
        mentionnez ne peut être traitée ...");
    }
    switch($superglobale){
        case "POST":
    ...
    foreach($array_name as $key_name => $value){
        ...
        // on définit une variable globale de façon à ce qu'elle 
        // puisse être utilisée en dehors de la fonction 
        // (cf: la portée des variables)
        ...
        Cela signifie qu'une variable est créée ou modifiée en 
        utilisant une chaîne de caractères comme nom.
        ...
        Par exemple, si la première itération de la boucle est 
        "nom" et que la valeur de $prefix est "test", 
        alors la variable créée sera "test_nom".
        L'avantage de cette méthode est que l'on 
        peut créer des variables dynamiquement sans 
        avoir à les déclarer explicitement.
        */
    }
}

function addInput($label, $properties, $required = false, $div_class = ""){
    // vérification d'une propriété name. 
    // Si non spécifié, le script s'arrête et génère une erreur
    ...
    // initialisation à true (est valide) => 
    // passera à false si le champ doit être vérifié et qu'il est vide
    $check  = true;
    // 0initialisation de la variable error qui contiendra 
    ...
    // si le paramètre $required vaut true, 
    // vérification du contenu de la value du champ
    if($required){
        ...
        // cette information sera renvoyée à la fin de l'exécution 
        // de la fonction
    }

    // création du html de l'input en rapport avec 
...
(spécifiées dans un tableau associatif) 
et éventuellement une classe CSS pour la balise DIV
...
/*
La fonction form est utilisée pour créer 
un formulaire HTML et pour valider les données 
saisies dans les champs de ce formulaire.

    -   Le paramètre $id est utilisé pour donner 
    un identifiant unique au formulaire.
    -   Le paramètre $action est utilisé pour définir 
    l'URL vers laquelle les données saisies dans 
    le formulaire seront envoyées lors de la soumission.
    -   Le paramètre $method est utilisé pour 
    définir la méthode HTTP utilisée pour 
    envoyer les données (GET ou POST).
    -   Le paramètre $content est un tableau qui 
    contient tous les éléments qui composeront 
    le formulaire, tels que les champs de saisie, les labels,
        les boutons d'envoi, etc.
    -   Le paramètre $class est optionnel et permet 
    de définir une classe CSS pour le formulaire.

La fonction vérifie d'abord si les données ont 
été soumises en utilisant la superglobale 
$_POST ou $_GET en fonction de la méthode choisie.
...
en utilisant les fonctions de validation de données définies précédemment.
Si un champ est vide et qu'il est requis, 
ou si une donnée saisie est incorrecte, 
un message d'erreur est affiché à côté de ce champ.
...
function form($id, $action, $method, $content, $class = ""){
    ...
    // boucle sur les différents éléments constituant 
    // le formulaire mise en forme (html) et champs
    foreach($content as $c){
        if(isset($c["html"])){
            ...
            // affichage d'une classe différente suivant 
            // qu'il s'agisse d'un champ manquant ou d'un champ mal rempli
    ...
    // si au moins un true est trouvé dans le array show_form 
    // : création d'un message d'erreur général
    ...
    // si au moins un true est trouvé dans le array show_form 
    // : ré-affichage du formulaire MAIS avec le message d'erreur
    // si aucun true alors return de false et soumission complète du form
    return in_array(true, $show_form) ? $msg.$form : false;
}

/*
Cette fonction, addLayout, est utilisée pour 
ajouter du contenu html à l'intérieur du formulaire 
sans que ce ne soit des champs de formulaire.
